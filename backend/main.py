...
app = FastAPI(title="Bearing Intelligence API", version="2.0.0")
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
def json_value(value: Any):
    if pd.isna(value) or not np.isfinite(value) if isinstance(value, (float, np.floating)) else False:
        return None
...
def evaluate_labels(actual, predicted):
    actual = pd.to_numeric(actual, errors="coerce")
    mask = actual.notna()
    if not mask.any():
        return None
    actual = actual[mask].astype(int)
    predicted = pd.Series(predicted, index=mask.index)[mask].astype(int)
    ...
    return {
        "available": True,
        "label_column": str(actual.name or "label"),
...
def analyze_frame(frame: pd.DataFrame):
    numeric = frame.select_dtypes(include=np.number)
    correlation = numeric.corr().replace({np.nan: None}).round(5).to_dict() if not numeric.empty else {}
    result = {
        "rows": int(len(frame)),
        ...
        "model": {"ready": MODEL is not None, "name": type(MODEL).__name__ if MODEL else None, "required_features": FEATURE_COLUMNS, "error": ARTIFACT_ERROR},
        "evaluation": {"available": False, "reason": "Upload a labelled dataset to calculate dataset-specific metrics."},
        "predictions": None,
    }
    if MODEL is None:
        return result
    ...
    states = pd.Series(predictions, index=frame.index[valid]).map({0: "Healthy", 1: "Faulty / Degraded"}).fillna(pd.Series(predictions, index=frame.index[valid]).astype(str))
    prediction_frame = pd.DataFrame({"row": frame.index[valid].tolist(), "prediction": predictions.tolist(), "state": states.tolist()})
    ...
    if label_column:
        evaluation = evaluate_labels(frame.loc[valid, label_column], predictions)
        if evaluation:
            evaluation["label_column"] = str(label_column)
